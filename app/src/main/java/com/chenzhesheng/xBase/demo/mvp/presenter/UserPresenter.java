 ...
 */
package com.chenzhesheng.xBase.demo.mvp.presenter;
...
import com.chenzhesheng.xBase.di.scope.ActivityScope;
import com.chenzhesheng.xBase.integration.AppManager;
import com.chenzhesheng.xBase.mvp.BasePresenter;
import com.chenzhesheng.xBase.utils.PermissionUtil;
import com.chenzhesheng.xBase.utils.RxLifecycleUtils;
...
public class UserPresenter extends BasePresenter<UserContract.Model, UserContract.View> {
    ...
    public void requestUsers(final boolean pullToRefresh) {
        ...
        PermissionUtil.Companion.externalStorage(new PermissionUtil.RequestPermission() {
            @Override
            ...
            public void onRequestPermissionFailure(List<String> permissions) {
                getMRootView().showMessage("Request permissions failure");
                getMRootView().hideLoading();//隐藏下拉刷新的进度条
            }

            @Override
            public void onRequestPermissionFailureWithAskNeverAgain(List<String> permissions) {
                getMRootView().showMessage("Need to go to the settings");
                getMRootView().hideLoading();//隐藏下拉刷新的进度条
            }
        }, getMRootView().getRxPermissions(), mErrorHandler);
    }
    ...
    private void requestFromModel(boolean pullToRefresh) {
        ...
        getMModel().getUsers(lastUserId, isEvictCache)
                .subscribeOn(Schedulers.io())
                .retryWhen(new RetryWithDelay(3, 2))//遇到错误时重试,第一个参数为重试几次,第二个参数为重试的间隔
                .doOnSubscribe(disposable -> {
                    if (pullToRefresh)
                        getMRootView().showLoading();//显示下拉刷新的进度条
                    else
                        getMRootView().startLoadMore();//显示上拉加载更多的进度条
                ...
                .doFinally(() -> {
                    if (pullToRefresh)
                        ...
                        getMRootView().endLoadMore();//隐藏上拉加载更多的进度条
                })
                .compose(RxLifecycleUtils.Companion.bindToLifecycle(getMRootView()))//使用 Rxlifecycle,使 Disposable 和 Activity 一起销毁
